...
A utilização de arquiteturas modernas que suportam Injeção de Dependência (DI) permite que o padrão Singleton seja implementado de forma mais elegante e testável. Essa abordagem abstrai a complexidade da implementação manual do padrão (como o controle de construtor privado e o método de acesso estático), ao mesmo tempo que mitiga suas desvantagens históricas, como o acoplamento forte e a dificuldade em realizar testes unitários (REFACTORING GURU, 2025). Através da DI, o OrderOrchestratorService pode ter suas dependências facilmente "mockadas" (simuladas) em um ambiente de teste, e seu ciclo de vida é gerenciado de forma limpa pelo contêiner de serviços da aplicação.  
